...
/**
 * Class QuoteRemind
 * Remind SMS when remind customer to pay the order
 * @package D1m\Wesms\Model\Action
 */
class QuoteRemind extends AbstractModel
{
    ...
    {
        return $this->_getStoreConfig('sms_setting/sms_template/quote_remind_enable');
    }
    ...
    protected function _getSMSTemplate()
    {
        ...
        {
            return $this->_getStoreConfig('sms_setting/sms_template/quote_remind_sms_template');
        }
    ...
    public function _getSMSTemplateCode()
    {
        if ($this->_enableSendSMS())
        {
            return $this->_getStoreConfig('sms_setting/sms_template/quote_remind_sms_template_code');
        }
    ...
    {
        $order = $this->getOrder();
        $createdAtPrc = $this->timezone->date($order->getCreatedAt(), null)->format('Y-m-d H:i:s');
        $param = [
            'createdAtPrc' => $createdAtPrc
        ];
