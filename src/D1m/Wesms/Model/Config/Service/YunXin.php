...
class YunXin extends AbstractModel
{
     ...
     */
    const API_URL_GATEWAY = "http://120.55.197.77:1210/Services/MsgSend.asmx/SendMsg";
     ...
     * @return mixed|string
     */
    public function getYunXinCaptchaUsr()
    {
        return trim($this->_getStoreConfig('sms_setting/sms/yunxin_captcha_usr'));
    }
     ...
     * @return mixed|string
     */
    public function getYunXinCaptchaPwd()
    {
        return trim($this->_getStoreConfig('sms_setting/sms/yunxin_captcha_pwd'));
    }
    ...
    {
        if(!strlen($this->getYunXinCaptchaUsr()))
        {
            $this->setError(__('API username can not be empty.'));
            return false;
        }

        if(!strlen($this->getYunXinCaptchaPwd()))
        {
